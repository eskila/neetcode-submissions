class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        # left number is always smaller or equal to the right, so by adding a 3rd number in between,
        # we can find the one that adds to create a zero-sum (or find that there's no solution for that triplet)
        # (this number may be 0, which sums to zero when l == -r).
        # How do we make sure we're not adding any duplicate triplets?
        # A: When we find a result we move the pointers and skip over duplicate values,
        # that way the valid combinations are always unique
        result = []
        while(l < n - 1):
            num_l = nums[l]
            # l + m + r = 0
            ...
            while nums[l] == nums[l+1] and l < n - 1:
                l += 1
            # But always need to move the l pointer regardless
            l += 1
        return result
